...
class AzureBlobStorageIndexedFileLocation(IndexedFileLocation):
    ...
    def _create_sas_query(self, conn_str, container_name, blob_name, expires_in_seconds):
    ...
    def _generate_azure_blob_storage_sas(
        ...
        sas_query = self._create_sas_query(azure_creds,container_and_blob[0],container_and_blob[1],expires_in)
    ...
    def get_signed_url(
        ...
        azure_creds = get_value(
            config, "AZ_BLOB_CREDENTIALS", InternalError("Azure Blob credentials not configured")
        )
